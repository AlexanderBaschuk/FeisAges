...
function setScalesAndAxes() {
	...
	var hDomainRightNew = Math.ceil(hDomainRightOrig / 10.0) * 10;
...
function generateVis() {
	...
	svg.append("g")
		.attr("class", "label")
		.attr("transform", "translate(" + graph.paddingLeft + ", 0)")
		.call(yAxis);
	...
	svg.append("g")
		...
		.append("text")
		.append("tspan")
		.attr("dx", function(d) { return yearScale(d); })
		.attr("dy", graph.paddingTop + 15)
...
function updateBars(svg) {
	...
	svg.selectAll(".barG")
		...
		.attr("y", function (d) { return h - graph.paddingBottom - hScale(d.girlsTotal + d.boysTotal); })
		.attr("height", function (d) { return hScale(d.girlsTotal); });
		//.each("end", function () { updateYearLabels(svg); });
